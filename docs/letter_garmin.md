...
I am writing this as someone who switched to Garmin from Apple Watch — not reluctantly, but
deliberately. The hardware case was clear: battery life, sensor depth, training algorithms, and
the richness of the data backend are genuinely without peer. Firstbeat Analytics, the training
load model, VO2 max estimation, Body Battery — these are the products of serious, sustained
engineering investment, and they show. This holds across the Forerunner, Fenix, Epix, Venu,
and Instinct lines. The platform problem I am describing is not specific to a product line.
It is present wherever the Connect IQ SDK is the development surface.

Which is precisely why the user-facing layer is so frustrating to encounter.
...
The instinct when reading UX criticism is to think about colours, fonts, and icon sizes. That
is not what I am describing. The issue with the Forerunner platform is structural: the
information architecture has no coherent model, and the UI layer reflects that.
...
cross-referenced. This is not "multiple entry points to the same feature." It is the same
concept implemented independently in four different places because there is no canonical data
contract at the platform level. The user pays for this with confusion. The developer pays for
it with duplication.
...
because the system does not have one to offer. This is not about too many features. It is about
features that were added over time without a unifying architecture to absorb them.
...
structurally unused. This is not a rendering limitation — the Connect IQ SDK exposes arc and
polygon primitives. It is a design culture that never made the form factor a first-class
constraint. Every list that clips at the bezel is a small, visible signal to the user that
...
own Controls menu, configure your data screens. This is a reasonable power-user feature. It
becomes a design failure when it is the primary mitigation for defaults that do not serve
...
configuration breaks. That is not power-user loyalty. It is learned helplessness — and it
actively suppresses adoption among users who will not invest those hours.

**The result is a platform that punishes its own users for caring.** The people most likely to
buy a Forerunner 970 are also the people most likely to eventually exhaust their patience with
a navigation model they cannot internalise. COROS has demonstrated, with a simpler but
coherent UI, that this is not an inevitable trade-off between feature depth and usability.
It is a choice.
...
primitives, inconsistently, under memory pressure. This is not a developer quality problem.
...
elements should behave on a circular AMOLED screen, no radial layout grid, no guidance on
safe-area margins relative to the bezel. The inevitable result is that each product team
designs independently, and the accumulated inconsistency is what users experience as
...
grows. No generation has simplified the model — because simplifying it would require
acknowledging that the model has a problem.

---

## III. The Proposal

I am not asking for a visual redesign. I am proposing three specific changes, ordered by
implementation cost and impact:

**Proposal 1 — Define a circular design language (cost: design investment, zero engineering).**  
Publish a Forerunner / round-device design specification: radial layout grid, safe-area insets,
...
enforce. To demonstrate feasibility, I am building an open-source prototype of exactly this:
...
**Proposal 3 — Establish a canonical data model for system-level concepts (cost: high
engineering, high impact).**  
Weather, heart rate, training status, Body Battery — these should each have one canonical
...
1. A public commitment to a Garmin circular design language — even a versioned draft published
   to the developer portal would change the trajectory of the Connect IQ ecosystem.
...
cleanly is not a small gap. It is the most consequential product problem Garmin has. And it is
entirely solvable — not by adding more settings, but by building the architectural foundation
that makes defaults good enough that most users never need to open Settings at all.
...
GitHub: [to be added]  
*IQKit* specification: [to be linked]  
March 2026